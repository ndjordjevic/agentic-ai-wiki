...
- Homepage: 
- Fetched: 2026-07-01
- Final URL: https://github.com/nadimtuhin/claude-token-optimizer
...
### UNIVERSAL_SETUP.md (root)

Manual setup guide for pasting into Claude Code when you want more control than `cto init`. Walks through creating the optimized documentation structure interactively, with framework-specific pattern inclusion after answering Claude's questions.

### QUICK_START.md (root)

Repository-level quick start for contributors and users exploring the tool itself (distinct from the `.claude/QUICK_START.md` template file that `cto init` creates in target projects).

### templates/README.md

Documents the template files shipped for `cto init`: CLAUDE.md structure, `.claudeignore`, support files (COMMON_MISTAKES, QUICK_START, ARCHITECTURE_MAP), completion templates, maintenance guide, and hook scripts under `templates/hooks/`.

### examples/ (structure only at standard detail)

Framework-specific setup guides with top-5 critical mistakes per stack:
- README.md, angular.md, django.md, express.md, fastapi.md, go.md, laravel.md, nestjs.md, nextjs.md, nuxtjs.md, rails.md, spring-boot.md, svelte.md, vue.md
...
├── bin/                  # CLI entrypoint binaries
├── src/
│   ├── cli.js            # Main CLI router
│   ├── commands/         # One module per cto subcommand
│   │   ├── init.js       # Framework-aware project scaffolding
│   │   ├── measure.js    # Token measurement / baseline
│   │   ├── audit.js      # 19 structural health checks (--fix, --json)
│   │   ├── compress.js   # Deterministic CLAUDE.md compression
│   │   ├── prune.js      # Interactive stale-section removal
│   │   ├── diff.js       # Before/after token delta
│   │   ├── watch.js      # Live token dashboard
│   │   ├── hooks.js      # Hook template install/list/settings
│   │   └── update.js     # npm + content template updates
│   └── lib/              # Shared utilities (token counting, file ops)
├── templates/            # Scaffolding templates + 12 hook shell scripts
│   ├── hooks/            # Claude Code hook templates (PreToolUse, UserPromptSubmit, etc.)
│   ├── completion-template.md
│   └── maintenance-guide.md
├── examples/             # 13 framework-specific setup guides (listed above)
├── tests/                # Test suite
├── scripts/              # Build/utility scripts
├── init.sh               # Legacy/alternate init script
├── install.sh            # One-line global installer (creates `cto` alias)
├── package.json          # npm package `claude-token-optimizer`
├── UNIVERSAL_SETUP.md    # Manual interactive setup guide
