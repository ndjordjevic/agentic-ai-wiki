...
### Merge rules (updating `wiki/overview.md`)
...
3. If changed: overwrites the changed raw file(s), re-runs ingest steps 2–5 and 7 (updates wiki pages; skips step 6 log, inbox-move, and commit), appends a log entry `refresh: <slug> | content updated`. For unified pages, derive companion context from the existing source page's `raw_files:` list and pass it to ingest so the unified body is preserved. For multi-product umbrellas, build `products` as the **union of (a) existing subs reconstructed from `subpages:` + each sub's `product:` and `source_url:`, and (b) newly-discovered products parsed from the new raw file's `## Product:` sections** that are not already represented. New entries become new sub-pages (additive); existing subs whose products are missing in the new discovery are kept (logged as INFO).
...
5. Refresh shape rules: a multi-product umbrella **can gain new sub-pages on refresh** (additive — when the upstream gains a product, or the original ingest missed one). A multi-product umbrella **never loses** sub-pages on refresh (use `remove <sub-slug>` to drop a sub manually). Standalone or unified pages are **not** auto-promoted to multi-product even when discovery finds ≥2 products — that shape change still requires `remove + re-add` (logged as an INFO suggestion). Companion repos on unified pages are not added or dropped on refresh either. `raw_files:` and `companion_urls:` are always preserved as-is.
...
If lint Check #11 flags a split-product wiki (separate web and github source pages share `product:`), the new ingestion flow produces a single unified page per product. To consolidate:
