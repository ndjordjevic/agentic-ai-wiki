...
> **Phase 1 note:** In Claude Code, use the `/pin-llm-wiki` subcommands (`init`, `add`, `run`, `lint`, `remove`) to automate this workflow. In Copilot, Cursor, and other tools, follow the workflow steps below directly — they are fully self-contained.
...
**Step 9 — Git commit (if `auto_commit: false`).**
...
4. Removes `<!-- refresh -->` from the inbox line, appends `<!-- refreshed YYYY-MM-DD -->`, preserves `[ ]`/`[x]` state. If `auto_commit: true`: commits.
