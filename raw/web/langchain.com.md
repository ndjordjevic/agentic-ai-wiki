...
## Fetch log
- Inbox URL: https://www.langchain.com/
- Final URL: https://www.langchain.com/
- Fetched: 2026-04-29
- Pages: 22
- Mode: deep-multi-product
- Products discovered: 5
- Products: langsmith, fleet, langchain, langgraph, deepagents
...
- [Deploy your deep agent app to cloud](https://docs.langchain.com/langsmith/deployment-quickstart-da.md)
...
- [Agent platform comparison](https://docs.langchain.com/langsmith/fleet/comparison.md): Compare LangSmith Fleet with Claude Cowork, Amazon Quick, and Google Workspace Studio to choose the right enterprise agent platform for your team
...
- [Sandbox snapshots](https://docs.langchain.com/langsmith/sandbox-snapshots.md): Build and capture snapshots—the filesystem images every sandbox boots from.
...
- [Data locations](https://docs.langchain.com/oss/javascript/deepagents/data-locations.md): Where the Deep Agents CLI stores configuration, sessions, and customization files
...
- [Data locations](https://docs.langchain.com/oss/python/deepagents/data-locations.md): Where the Deep Agents CLI stores configuration, sessions, and customization files
...
## Discovery audit
- Candidates from llms.txt (depth 1): api-reference, langsmith, oss, index
- Candidates from llms.txt (depth 2 under oss): javascript, python
- Candidates from llms.txt (depth 3 under oss/python): common-errors, concepts, contributing, deepagents, integrations, langchain, langgraph, learn, migrate, reference, release-policy, releases, security-policy, versioning
- Candidates from llms.txt (depth 3 under oss/javascript): common-errors, concepts, contributing, deepagents, integrations, langchain, langgraph, learn, migrate, reference, release-policy, releases, security-policy, versioning
- Candidates from llms.txt (depth 3 under langsmith): fleet, abac, access-current-span, add-auth-server, add-human-in-the-loop, add-metadata-tags, administration-overview, agent-auth, agent-server, agent-server-api, ...
- Candidates from landing page: LangSmith (observability/evaluation/deployment platform), Fleet (no-code agents), LangChain (open-source framework), LangGraph (open-source runtime/orchestration)
- Candidates from concepts/products page: LangChain (agent framework), LangGraph (agent runtime), Deep Agents SDK (agent harness)
- Candidates from GitHub URLs: https://github.com/langchain-ai/deepagents (referenced in deepagents docs)
- Classified as products: langsmith (depth-1 llms.txt section, own docs cluster, landing page card), fleet (sub of langsmith docs but distinct landing page card, existing product), langchain (depth-3 under oss/python, own docs cluster, landing page mention), langgraph (depth-3 under oss/python, own docs cluster, landing page mention, product page), deepagents (depth-3 under oss/python + oss/javascript, extensive docs cluster, own GitHub repo, agent harness classification on concepts/products page — NEW since last ingest)
- Classified as excluded: api-reference (reference section not a product), oss (generic namespace container), python (generic namespace container), javascript (generic namespace container), index (root index page), concepts (sub-section), contributing (sub-section), integrations (sub-section), learn (sub-section), migrate (sub-section), reference (sub-section), release-policy (sub-section), releases (sub-section), security-policy (sub-section), versioning (sub-section), common-errors (sub-section)
- Classified as sub-sections of existing products: all agent-server-api/* paths (sub of langsmith), langsmith/fleet/* (sub of fleet), oss/python/langchain/* (sub of langchain), oss/python/langgraph/* (sub of langgraph), oss/python/deepagents/* (sub of deepagents), oss/javascript/* (JS equivalents of Python products)

## Landing page — https://www.langchain.com/
...
Improve agent performance across the development lifecycle. Trace your preferred framework or integrate LangSmith with any agent stack using Python, TypeScript, Go, or Java SDKs.

**Observability** — Tracing breaks each run into a structured timeline of steps so teams can see exactly what happened, in what order, and why. Native tracing for popular agent frameworks and OpenTelemetry; SDKs for Python, TypeScript, Go, and Java; message threading for multi-turn chat; analytics and AI-driven insights.

**Evaluation** — Capture production traces, turn them into test cases, and score agents with human review and automated evals. Reusable LLM-as-judge and multi-turn evals; eval calibration with human feedback; online and offline scoring.

**Deployment** — Agent server provides memory, conversational threads, and durable checkpointing. Supports human-in-the-loop interactions, input concurrency, background agents; type-safe streaming; scalable distributed runtime; native A2A and MCP support.

**Fleet** — Routine tasks like research, follow-ups, and status checks. Bring your own models; use first-party integrations or extend with any MCP server; export agent files for pro-code development; integrated LangSmith tracing; agents improve with user feedback.
...
LangChain is the platform for agent engineering. AI teams trust LangChain's products to engineer reliable agents.

**LangSmith** is a platform that helps AI teams use live production data for continuous testing and improvement — tracing, evaluation, deployment, and Fleet.

**Open source agent frameworks** — Python and TypeScript, covering LangChain and LangGraph.

Documentation index available at: https://docs.langchain.com/llms.txt

## Concepts — Products — https://docs.langchain.com/oss/python/concepts/products
LangChain maintains three layers in the open-source agent stack:
- **Agent frameworks** (like LangChain): Abstractions, integrations. For getting started quickly or standardizing how a team builds.
- **Agent runtimes** (like LangGraph): Durable execution, streaming, HITL, persistence, low-level control. For long-running, stateful workflows.
- **Agent harnesses** (like the Deep Agents SDK): Predefined tools, prompts, subagents. For more autonomous agents and complex, non-deterministic tasks.

Other examples in the ecosystem: frameworks include Vercel AI SDK, CrewAI, OpenAI Agents SDK, Google ADK, LlamaIndex; runtimes include Temporal, Inngest; harnesses include Claude Agent SDK, Manus.

## Product: LangSmith
- Slug: langsmith
- Deep link: https://www.langchain.com/langsmith
- Docs URL: https://docs.langchain.com/langsmith/home
- Companion repo: n/a

### About
LangSmith is positioned as LangChain's framework-agnostic platform for building, debugging, and deploying AI agents and LLM applications. The public site emphasizes visibility into agent behavior, monitoring, and production debugging, while the docs describe one workflow that combines observability, evaluation, deployment, and platform setup from local development through production.

### Docs — https://docs.langchain.com/langsmith/home
LangSmith is a framework-agnostic platform for building, debugging, and deploying AI agents and LLM applications. It supports tracing, evaluation, prompt and test workflows, and deployment with Python, TypeScript, Go, and Java SDKs.

### Evaluation — https://docs.langchain.com/langsmith/evaluation
LangSmith supports both offline evaluation on datasets before deployment and online evaluation on live production traces. The documented workflow is dataset creation, evaluator definition, running experiments, analyzing results, then feeding failing production traces back into datasets and redeploying.

### Deployment — https://docs.langchain.com/langsmith/deployment
LangSmith Deployment is a workflow orchestration runtime for agent workloads. It supports cloud, hybrid, and self-hosted deployment options, RemoteGraph composition, MCP and A2A support, and Studio for developing and debugging local or deployed agents.

### Agent Server — https://docs.langchain.com/langsmith/agent-server
Agent Server exposes assistants, threads, runs, cron jobs, persistence, and a task queue. It deploys graphs plus a database and queue, persists checkpoints and long-term memory, and supports single-host, split API/queue, and distributed runtime modes with PostgreSQL-backed persistence and Redis for signaling and streaming.

## Product: Fleet
- Slug: fleet
- Deep link: n/a
- Docs URL: https://docs.langchain.com/langsmith/fleet
- Companion repo: n/a

### About
Fleet is presented on the public site as a way to turn plain-language requests into recurring or interactive company-facing agents. The docs describe it as a no-code platform for creating and managing AI agents with approvals, integrations, channels, memory, templates, and organization-level data handling inherited from LangSmith settings.

### Docs — https://docs.langchain.com/langsmith/fleet
LangSmith Fleet is a no-code platform for creating and managing AI agents. It can automate recurring work, connect external apps, run inside chat surfaces like Slack, and keep humans in control through approval steps.

### Essentials — https://docs.langchain.com/langsmith/fleet/essentials
Fleet essentials are tools, channels, memory, sub-agents, approvals, instructions, traces, and custom models. Long-term memory can live in files like `AGENTS.md`, `tools.json`, `subagents/*`, and `skills/*`, while agents can also persist memories by writing files at runtime.

### Tools — https://docs.langchain.com/langsmith/fleet/tools
Fleet tools can be added from the workspace integrations tab or per-agent in the editor. The documented categories include email, calendars, chat, project management, code hosting, spreadsheets and BI, search, social, general web utilities, and remote MCP servers.

### Templates — https://docs.langchain.com/langsmith/fleet/templates
Fleet includes starter templates with pre-configured tools, instructions, and optional channels. Templates can be cloned and customized by changing prompts, tools, channels, and models for specific business workflows.

## Product: LangChain
- Slug: langchain
- Deep link: n/a
- Docs URL: https://docs.langchain.com/oss/python/langchain/overview
- Companion repo: n/a

### About
LangChain is the higher-level open source framework in the LangChain stack. The docs describe it as a fast way to build custom agents and applications in under 10 lines of code, with model-provider integrations, prebuilt agent architecture, tools, middleware, and optional LangSmith tracing.

### Docs — https://docs.langchain.com/oss/python/langchain/overview
LangChain helps developers build custom agents and LLM applications quickly with provider integrations and prebuilt agent architecture. The overview points new users to installation, quickstart, and agents, and frames LangSmith as the preferred tracing surface.

### Quickstart — https://docs.langchain.com/oss/python/langchain/quickstart
The quickstart walks through installing dependencies, setting provider API keys, building a basic tool-using agent, then extending to a research-oriented agent with prompts, tools, model configuration, memory, testing, tracing, and optional Deep Agents features.

### Agents — https://docs.langchain.com/oss/python/langchain/agents
LangChain agents combine models and tools through `create_agent`. The agents docs cover static and dynamic model selection, static and dynamic tool registration, middleware hooks such as `wrap_model_call` and `wrap_tool_call`, and runtime patterns for permissions, feature flags, and state-aware tool exposure.

## Product: LangGraph
- Slug: langgraph
- Deep link: https://www.langchain.com/langgraph
- Docs URL: https://docs.langchain.com/oss/python/langgraph/overview
- Companion repo: n/a

### About
LangGraph is the lower-level orchestration framework and runtime in the LangChain stack. The public product page emphasizes durable, controllable agents with human-in-the-loop, memory, streaming, and flexible multi-agent control flow; the docs frame it as the orchestration layer underneath long-running, stateful agents.

### Docs — https://docs.langchain.com/oss/python/langgraph/overview
LangGraph is a low-level orchestration framework and runtime for long-running, stateful agents. Core benefits include durable execution, human-in-the-loop, comprehensive memory, debugging via LangSmith, and production-ready deployment.

### Workflows and agents — https://docs.langchain.com/oss/python/langgraph/workflows-agents
The workflows guide distinguishes predetermined workflows from dynamic agents and documents common patterns such as prompt chaining, parallelization, routing, orchestrator-worker, and evaluator-optimizer loops. It uses `StateGraph`, tool binding, and `Send` to express these flows.

### Interrupts — https://docs.langchain.com/oss/python/langgraph/interrupts
Interrupts let a graph pause execution, persist state through a checkpointer, and resume later with external input via `Command(resume=...)`. The docs show approval workflows, multiple parallel interrupts, streamed human-in-the-loop handling, and thread-based resumption semantics.

## Product: Deep Agents
- Slug: deepagents
- Deep link: n/a
- Docs URL: https://docs.langchain.com/oss/python/deepagents/overview
- Companion repo: https://github.com/langchain-ai/deepagents

### About
Deep Agents is LangChain's batteries-included "agent harness" — an opinionated, higher-level SDK built on top of LangGraph that adds planning capabilities, file-system context management, subagent spawning, and long-term memory out of the box. It also ships a terminal coding agent (Deep Agents CLI) built on the same SDK. The docs classify it as a harness distinct from frameworks (LangChain) and runtimes (LangGraph), and position it for complex, multi-step tasks that require autonomous planning and decomposition.

### Overview — https://docs.langchain.com/oss/python/deepagents/overview
The Deep Agents SDK is the easiest way to start building agents powered by LLMs with built-in capabilities for task planning, file systems for context management, subagent-spawning, and long-term memory. It is a standalone library built on top of LangChain's core building blocks and uses the LangGraph runtime for durable execution, streaming, and human-in-the-loop. The repo contains the Deep Agents SDK package, the Deep Agents CLI (a terminal coding agent), and an ACP integration for use in code editors like Zed.

Use the Deep Agents SDK when you want agents that can: handle complex multi-step tasks with planning and decomposition; manage large amounts of context through file system tools and summarization; swap filesystem backends (in-memory, local disk, durable stores, sandboxes, or custom); execute shell commands via a sandbox backend; delegate work to specialized subagents for context isolation; persist memory across conversations; control filesystem access with declarative permission rules; require human approval for sensitive operations; or use any model provider.

### Quickstart — https://docs.langchain.com/oss/python/deepagents/quickstart
Create a deep agent with `create_deep_agent(model=..., tools=[...], system_prompt=...)`. The model string uses `provider:model` format (e.g. `google_genai:gemini-3.1-pro-preview`, `openai:gpt-5.4`, `anthropic:claude-sonnet-4-6`). The agent automatically: plans its approach using `write_todos`; conducts tool calls; manages context by writing large results to file system tools (`write_file`, `read_file`); spawns subagents as needed; and synthesizes a final response. Invoke with `agent.invoke({"messages": [{"role": "user", "content": "..."}]})`. Built-in streaming support via LangGraph.

### CLI — https://docs.langchain.com/oss/python/deepagents/cli
The Deep Agents CLI is an open-source terminal coding agent built on the Deep Agents SDK. Built-in tools include: `ls`, `read_file`, `write_file`, `edit_file`, `glob`, `grep`, `execute` (shell), `web_search` (Tavily), `fetch_url`, `task` (subagent delegation), `ask_user`, `compact_conversation` (context offloading), and `write_todos`. Destructive operations require human approval by default (`-y` to auto-approve). The CLI supports slash commands (`/model`, `/agents`, `/remember`, `/skill:<name>`), MCP server tools, persistent memory across sessions, custom skills, and LangSmith tracing. Providers: OpenAI, Anthropic, Google included out of the box; others installable separately via `uv tool install deepagents-cli --with langchain-<provider>`.
