...
## Metadata
- Stars: 666
- Primary language: (none — skill/markdown package)
- Default branch: main
- Latest release: (none)
- License: Other (MIT per README/LICENSE)
- Homepage: (none)
- Fetched: 2026-07-14
- Final URL: https://github.com/chaseai-yt/grill-me-codex

## Description
(none on GitHub — see README)
...
Built on Matt Pocock's `grill-me` / `grill-with-docs` skills (MIT, `github.com/mattpocock/skills`) — Act 1 is his work; the iterative cross-model Codex review (Act 2) and the role-flipped build (Act 3) are the additions. Act 3's delegation pattern is adapted from Peter Steinberger's `codex-first` (`github.com/steipete/agent-scripts`).
...
**Review skills (Acts 1–2):** Codex runs **read-only every round** — `-s read-only` on the first call, `-c sandbox_mode="read-only"` on every resume (the `resume` subcommand doesn't accept `-s`, and without forcing read-only it would inherit the user's `config.toml` sandbox default, which may be `danger-full-access`). The skills handle this for the user. No code is ever written until the plan is approved.
...
- Act 1 (`grill-me`, `grill-with-docs`) © Matt Pocock — `github.com/mattpocock/skills` (MIT).
- Act 3's Codex-as-builder pattern adapted from Peter Steinberger's `codex-first`.
- Act 2 (iterative Codex review), Act 3 (codex-build), and packaging by Chase AI (`youtube.com/@chaseai`).
...
- `skills/` — four Claude Code skills, each in its own folder: `grill-me-codex/`, `grill-with-docs-codex/`, `codex-review/`, `codex-build/`
- `README.md` — full methodology and safety documentation (captured above)
- `LICENSE` — MIT
