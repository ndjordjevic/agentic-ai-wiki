...
- Primary language: Go
- Default branch: main
- Latest release: v0.31.2 (2026-07-06, pre-release)
- License: MIT License
- Homepage: https://ollama.com
- Fetched: 2026-07-07
- Final URL: https://github.com/ollama/ollama
...
Model names follow a `model:tag` format, where `model` can have an optional namespace such as `example/model`. Some examples are `orca-mini:3b-q8_0` and `llama3:70b`. The tag is optional and, if not provided, will default to `latest`.
...
Structured outputs are supported by providing a JSON schema in the `format` parameter. JSON mode is enabled by setting `format` to `json`.

### Multimodal

Multimodal models (e.g. `llava`) accept base64-encoded `images` in generate/chat requests.

### Thinking models

The `think` parameter controls whether thinking models reason before responding (boolean or levels: `low`, `medium`, `high`, `max`).
...
| Path | Type | Notes |
|---|---|---|
| `agent/` | dir | Agent integration helpers |
| `anthropic/` | dir | Anthropic-compatible API surface |
| `api/` | dir | REST API route handlers |
| `app/` | dir | Desktop application code |
| `auth/` | dir | Authentication |
| `cmd/` | dir | CLI entry points (`ollama` binary) |
| `convert/` | dir | Model format conversion |
| `discover/` | dir | Model discovery |
| `docs/` | dir | Documentation source (CLI, API, integrations, modelfile) |
| `integration/` | dir | Third-party coding-agent integrations (Claude Code, Codex, OpenClaw, etc.) |
| `internal/` | dir | Internal packages |
| `llama/` | dir | llama.cpp native bindings |
| `llm/` | dir | LLM inference runtime |
| `ml/` | dir | MLX engine (safetensors models) |
| `model/` | dir | Model manifest and lifecycle |
| `openai/` | dir | OpenAI-compatible API surface |
| `runner/` | dir | Inference runner |
| `server/` | dir | HTTP server (default port 11434) |
| `template/` | dir | Prompt templates |
| `thinking/` | dir | Thinking-model support |
| `tools/` | dir | Tool-calling support |
| `types/` | dir | Shared types |
| `main.go` | file | Go entry point |
| `Dockerfile` | file | Official Docker image build |
| `AGENTS.md` | file | Agent build instructions |
| `CLAUDE.md` | file | Claude Code project instructions |
| `.github/` | dir | CI/workflows (boilerplate) |
