...
- Latest release: v0.0.19 (2022-04-16)
- License: Other (code under GPL v3; prose under custom restrictions — see LICENSE)
- Homepage: 
- Fetched: 2026-07-02
- Final URL: https://github.com/fastai/fastbook
...
| Path | Type | Notes |
|---|---|---|
| `01_intro.ipynb` … `20_conclusion.ipynb` | notebooks | 20 main book chapters covering intro → production → ethics → vision → tabular → NLP → mid-level API → convolutions → ResNet → optimizers → foundations → GradCAM → Learner API → conclusion |
| `app_jupyter.ipynb` | notebook | Jupyter introduction appendix |
| `app_blog.ipynb` | notebook | Blog appendix |
| `utils.py` | Python | Shared notebook utilities: fastai imports, plotting helpers, Bing/DuckDuckGo image search, graphviz helpers |
| `tools/clean.py` | Python | nbdev export script — strips notebook cells for "clean" chapter copies in `clean/` |
| `clean/` | directory | Cleaned notebook copies (same chapter filenames, symlinked `images/`) |
| `images/` | directory | Figures and assets referenced by notebooks |
| `translations/` | directory | Community translations (e.g. `cn/` Chinese) |
| `environment.yml` | config | Conda environment pinning pytorch + fastai channels |
| `requirements.txt` | config | pip dependencies (fastai, nbdev, scikit-learn, sentencepiece, etc.) |
| `settings.ini` | config | nbdev project metadata; docs published to `fastai.github.io/fastbook/` |
| `README_*.md` | docs | Localized README variants (ar, bn, es, id, it, ja, ko, pt, tr, vn, zh) |
| `LICENSE` | legal | GPL v3 for code; custom restrictions on prose |
| `CODE_OF_CONDUCT.md` | community | Contributor code of conduct |
| `.github/` | boilerplate | (skipped) |
