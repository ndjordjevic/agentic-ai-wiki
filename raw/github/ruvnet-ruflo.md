...
- Default branch: main
- Latest release: v3.16.3 (2026-07-01)
- License: MIT License
- Homepage: https://Cognitum.One
- Fetched: 2026-07-03
- Final URL: https://github.com/ruvnet/ruflo
...
### docs/USERGUIDE.md — Architecture & Modules (excerpt)
...
### docs/USERGUIDE.md — Core Features (excerpt)
...
Queen types: Strategic (research/planning), Tactical (implementation), Adaptive (optimization).

Consensus mechanisms: Byzantine (2/3 supermajority), Raft (leader-elected), Quorum, Gossip, CRDT.

Collective memory types: knowledge (permanent), context (1h TTL), task (30min TTL), result (permanent), error (24h TTL), metric (1h TTL), consensus (permanent), system (permanent).

```bash
npx ruflo hive-mind init
npx ruflo hive-mind spawn "Build API" --queen-type strategic --consensus byzantine
npx ruflo hive-mind status
```

#### Quick Start prerequisites

- Node.js 20+, npm 9+ / pnpm / bun
- Claude Code installed globally: `npm install -g @anthropic-ai/claude-code`
- Install: `npx ruflo@latest init` or `curl -fsSL https://cdn.jsdelivr.net/gh/ruvnet/ruflo@main/scripts/install.sh | bash`
- MCP: `claude mcp add ruflo -- npx ruflo@latest mcp start`

### docs/metaharness-user-guide.md (excerpt)

MetaHarness integration in ruflo 3.12.1+. Ten CLI subcommands, nine MCP tools — scores harness readiness, maps MCP surfaces, threat-models, fingerprints genome characteristics, detects drift. Strictly optional; degrades gracefully when `@metaharness/*` packages are uninstalled.
...
npx ruflo eject  # turn a ruflo project into a standalone agent toolkit
```

### AGENTS.md (excerpt — orchestrator vs executor model)
...
Workflow: `memory_search` → learn past patterns; `swarm_init` → coordination record; agent executes work; `memory_store` → remember patterns.

Division of labor: CODEX executes (write files, run tests); claude-flow orchestrates (track agents, store memory, coordinate tasks). After any claude-flow command, the executor must immediately continue with actual work — coordination commands return instantly and create records only.

## Top-level structure

| Path | Type | Notes |
|------|------|-------|
| `.agents/` | dir | Agentics Foundation config (Codex CLI) |
| `.claude-plugin/` | dir | Claude Code plugin manifests |
| `.claude/` | dir | Claude Code project settings (created by init) |
| `.githooks/` | dir | Git hooks |
| `.github/` | dir | CI/workflows (skip detail) |
| `.harness/` | dir | Harness claims and metaharness integration |
| `AGENTS.md` | file | Agent instructions for Codex/OpenAI harnesses |
| `CLAUDE.md` | file | Claude Code project instructions |
| `CHANGELOG.md` | file | Release history |
| `Cargo.toml` | file | Rust workspace root (RuVector/Cognitum engine) |
| `bin/` | dir | CLI binaries |
| `crates/` | symlink | Rust crates |
| `data/` | dir | Clone/download proof ledgers |
| `docs/` | dir | USERGUIDE, STATUS, metaharness guide, federation, benchmarks, ADRs |
| `plugin/` | dir | Plugin packaging |
| `plugins/` | dir | 35 native Claude Code plugins (ruflo-core, ruflo-swarm, ruflo-rag-memory, etc.) |
| `ruflo/` | dir | Main TypeScript monorepo source (v3 packages, ruVocal web UI) |
| `scripts/` | dir | install.sh, benchmarks |
| `tests/` | dir | Test suites |
| `v3/` | dir | Goal UI (goal.ruv.io), assets |
| `verification/` | dir | Cryptographic verify (`ruflo verify`) |
