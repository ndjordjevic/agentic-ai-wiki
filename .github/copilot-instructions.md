...
- `product:` groups sources for the same product (marketing site + own GitHub repo, etc.). Resolved in Step 2b. Default `null`.
...
- **GitHub source:** read `homepageUrl` from the raw `## Metadata` block. Strip `www.` from the hostname. If any existing **web** source page slug matches that hostname, set `product:` on both pages to that hostname.
- **Web source:** scan the raw file body for a `github.com/<org>/<repo>` URL in the first ~500 chars or in title/hero. If `<org>-<repo>` matches an existing **github** source slug, set `product:` on both pages to the web source's slug (the domain).
- **YouTube:** never auto-grouped.
- **No match:** leave `product: null`. Humans may set `product:` manually for cases the heuristic misses.
...
**Invariant:** the body contains **exactly one paragraph per entry in `sources:`**, in the same order. Verify before writing.
