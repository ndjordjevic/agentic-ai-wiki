...
- Default branch: main
- Latest release: v0.0.75 (2026-05-08)
...
## Description
Playwright MCP server — provides browser automation capabilities using Playwright for LLMs, enabling web interaction through structured accessibility snapshots.
...
## Top-level structure
- `.claude/` — dir (agent instruction files: CLAUDE.md skills)
- `.devcontainer/` — dir (dev container config)
- `.github/` — dir (CI workflows, issue templates — boilerplate)
- `.gitignore` — file
- `.npmignore` — file
- `CLAUDE.md` — file (**agent instruction file**: commit convention, rolling Playwright, release prep)
- `CONTRIBUTING.md` — file (contributor guide; references source in Playwright monorepo)
- `Dockerfile` — file (Docker image for headless Chromium deployment)
- `LICENSE` — file (Apache-2.0)
- `README.md` — file (primary documentation — full tool list, config, client setup)
- `SECURITY.md` — file
- `cli.js` — file (CLI entry point)
- `config.d.ts` — file (TypeScript type declarations for config schema)
- `index.d.ts` — file (main TypeScript declarations)
- `index.js` — file (main entry point)
- `package.json` — file (v0.0.75, name: @playwright/mcp)
- `playwright.config.ts` — file (test configuration)
- `roll.js` — file (Playwright dependency bump script)
- `server.json` — file (MCP server manifest)
- `src/` — dir (source is in Playwright monorepo; src/README.md redirects there)
- `tests/` — dir (end-to-end tests)
- `update-readme.js` — file (regenerates tool list and config schema in README)
...
4. Commit with `chore: roll Playwright to <version>`, push, and open a PR.
