...
- Fetched: 2026-09-19
- Final URL: https://github.com/affaan-m/ECC
...
## Top-level structure
- `AGENTS.md`, `CLAUDE.md` — cross-tool and Claude-specific agent instructions (see Docs above)
- `README.md`, `README.zh-CN.md` — English + Chinese README; 11 more localized READMEs linked from the top of README.md
- `SOUL.md`, `SECURITY.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `CHANGELOG.md`, `TROUBLESHOOTING.md`, `COMMANDS-QUICK-REF.md` — top-level project docs
- `the-shortform-guide.md`, `the-longform-guide.md`, `the-security-guide.md` — the three main narrative guides referenced throughout the README
- `agents/` — 68 specialized subagent definitions (Markdown + YAML frontmatter): planner, architect, tdd-guide, code-reviewer, security-reviewer, per-language reviewers/build-resolvers (Go, Python, Java, Rust, Kotlin, C++, F#, TypeScript, HarmonyOS/ArkTS), mle-reviewer, rag-pipeline-reviewer, harness-optimizer, loop-operator, etc.
- `skills/` — 292 reusable workflow skills (SKILL.md files): coding-standards, tdd-workflow, security-review, continuous-learning-v2 (instinct-based learning), unified-memory (Memory Vault), ito-compute (GPU compute RFQ bridge), per-framework packs (Django, Laravel, Spring Boot/Quarkus, Rails), per-language packs (Go, Python, Swift, Perl, C++), configure-ecc (install wizard), skill-stocktake, autonomous-loops, and more
- `commands/` — 94 maintained slash-command shims (being superseded by skills); `legacy-command-shims/` holds retired short names (`/tdd`, `/eval`, `/e2e`, `/verify`, `/orchestrate`)
- `rules/` — always-loaded standards: `common/` (coding-style, git-workflow, testing, performance, patterns, hooks, agents, security) plus per-language dirs (typescript, python, golang, swift, php, arkts)
- `hooks/` — `hooks.json` (PreToolUse/PostToolUse/Stop/etc.), `memory-persistence/`, `strategic-compact/`
- `scripts/` — cross-platform Node.js installer/repair/sync/orchestration scripts (`ecc.js`, `uninstall.js`, `sync-ecc-to-codex.sh`, `codex/check-plugin-cache.js`, `hooks/*.js`)
- `.claude-plugin/` — Claude Code marketplace manifest (`plugin.json`, `marketplace.json`)
- `.codex/`, `.codex-plugin/` — Codex reference config (`config.toml`, `agents/*.toml` roles: explorer, reviewer, docs_researcher) and native Codex plugin
- `.cursor/`, `.opencode/`, `.zed/`, `.gemini/`, `.qwen/`, `.kimi/`, `.hermes/`, `.openclaw/`, `.codebuddy/`, `.kiro/`, `.pi/`, `.trae/`, `.adal/` — per-harness adapters of varying maturity (stable/beta/experimental, see the Platform Support table)
- `.github/` — `copilot-instructions.md` (GitHub Copilot instruction layer) and `prompts/` (`/plan`, `/tdd`, `/security-review`, `/build-fix`, `/refactor` prompt files)
- `contexts/` — dynamic system-prompt injection contexts (dev, review, research)
- `examples/` — example `CLAUDE.md` configs, including real-world stack examples (SaaS Next.js+Supabase+Stripe, Go microservice, Django REST API, Laravel API, Rust API)
- `mcp-configs/mcp-servers.json` — bundled MCP server definitions (GitHub, Context7, Exa, Memory, Playwright, Sequential Thinking, Supabase, etc.); only `chrome-devtools` is enabled by default
- `docs/` — setup/architecture/operating guides (not expanded at standard detail; README links the key ones: CODEX-NAVIGATION-GUIDE, MANUAL-ADAPTATION-GUIDE, MCP-CONNECTOR-POLICY, ROADMAP, token-optimization, releases/)
- `manifests/`, `schemas/`, `scaffolds/`, `plugins/`, `integrations/`, `config/`, `docker/`, `ecc2/`, `src/`, `workflows/`, `tests/` — supporting build/runtime/test directories (not expanded at standard detail)
- `ecc_dashboard.py` — optional desktop Tkinter GUI for browsing agents/skills/commands/rules
- `package.json`, `package-lock.json`, `yarn.lock`, `.tool-versions`, `pyproject.toml`, `eslint.config.js`, `commitlint.config.js`, `.mcp.json`, `.env.example` — tooling/config
