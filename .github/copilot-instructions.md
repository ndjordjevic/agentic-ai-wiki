...
This wiki is the authoritative source for this domain. Training-data knowledge about agentic AI frameworks is noise here; the wiki is signal.
...
- Then: sectioned body. Per-type structure (canonical sections, in this order; extra top-level sections allowed only for genuinely substantial subsystems — fold incidental content into a listed section):
...
- **Populate `tags` and `related` at ingest time.**
  - `tags`: 4–8 short kebab-case terms describing the source's main concepts, technologies, and domains. Specific over generic — prefer `claude-code-hooks`, `provider-adapter` over `ai`, `tool`.
  - `related`: slugs of existing source pages with substantial conceptual overlap (same problem space, shared architecture, same underlying tool). Read each existing source's summary paragraph to judge overlap. May be empty if this is the first source or no overlap exists.
  - **Bidirectional update:** for every slug `Y` added to this page's `related`, append this page's slug to `Y`'s `related` list and bump `Y`'s `updated:` date. Otherwise older pages never gain forward links to newer ones.
- Add per-paragraph citations only when a **second** raw file contributes to this page; otherwise the banner covers it.

**Unified body structure** (web source with companion GitHub):
...
The banner cites the primary web raw file. All github-sourced paragraphs carry trailing inline citations `(../../raw/github/<org>-<repo>.md)`. Both formats satisfy citation rule #9 (relative-from-file).
...
- **Web source:** scan the raw file body for a `github.com/<org>/<repo>` URL in the first ~500 chars or in title/hero. If `<org>-<repo>` matches an existing **github** source slug, set `product:` on both pages to the web source's slug (the domain). Otherwise derive from the domain (strip `.com`/`.io`/etc, e.g. `runcabinet.com` → `cabinet`).
- **YouTube:** derive from the video title or channel name (short, kebab-case). Never auto-grouped with other sources.
...
```
For unified pages, extend the Updated line and add:
```
...
**Unified source pages** (web + companion GitHub) — same as above, plus two optional fields:
```yaml
---
type: source
...
| 4 | Data gaps — concept in ≥`topic_min_products` (default 3) distinct products with no topic page | INFO |
...
| 12 | Split-product sources — web and github source pages share `product:` but lack `companion_urls` | WARN |

Auto-fixes applied on every lint run:
- Missing `overview.md` / `log.md` links in `wiki/index.md`.
- Re-sync `.cursor/rules/wiki-instructions.mdc` and `.github/copilot-instructions.md` from `AGENTS.md` when Check #11 detects drift (preserves the Cursor file's YAML frontmatter).

Topic candidates from Check #4 are **reported only** by default (`topic_creation: report`) — promote them to real topic pages via the manual harvest flow below. Set `topic_creation: auto-stub` in `.pin-llm-wiki.yml` to restore the legacy behavior of auto-creating empty stub topic pages.

**Source independence:** Check #4 counts distinct *products*, not distinct sources. Sources sharing a `product:` frontmatter value (e.g. a marketing site + its own GitHub repo) count as one product, so a feature shared between them does not falsely trigger a topic candidate.
...
4. Run lint immediately: flags orphaned `[[wikilinks]]` on surviving pages and pages whose `sources:` frontmatter still references `[[<slug>]]`.
5. Report to user; do not auto-rewrite the surviving pages flagged by lint.
...
If lint Check #12 flags a split-product wiki (separate web and github source pages share `product:`), the new ingestion flow produces a single unified page per product. To consolidate:

1. `/pin-llm-wiki remove <github-slug>` to soft-delete the standalone github source page (raw file is archived too).
2. Re-add the web URL with `/pin-llm-wiki run` (or queue + run). The web fetch protocol's companion-discovery step will pick up the GitHub repo automatically and the ingest will produce a unified source page citing both raw files.
3. Run `/pin-llm-wiki lint` to verify no dangling `[[<github-slug>]]` references remain.

If you prefer not to re-fetch, edit by hand:
1. Add `companion_urls` and `raw_files` frontmatter to the web source page.
2. Merge github body sections into the web page using inline citations `(../../raw/github/<org>-<repo>.md)`.
3. Move `wiki/sources/<github-slug>.md` → `wiki/.archive/sources/`. Keep `raw/github/<github-slug>.md` in place — it is now the companion raw.
4. Edit `wiki/index.md`: remove the github row, decrement source count.
5. Edit `wiki/overview.md`: remove `[[<github-slug>]]` from `sources:` list; delete its paragraph (the invariant is `len(sources) == paragraph_count`).
6. On any source page whose `related:` references `[[<github-slug>]]`, replace with `[[<web-slug>]]` (or remove if `[[<web-slug>]]` is already there).
