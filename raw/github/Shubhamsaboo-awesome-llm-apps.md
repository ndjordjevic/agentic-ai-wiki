...
- Fetched: 2026-07-17
- Final URL: https://github.com/Shubhamsaboo/awesome-llm-apps
...
Featured on Trendshift as the #1 repository of the day (repo 9876).
...
*   [🌐 Openwork - Open Browser Automation Agent](https://github.com/accomplish-ai/openwork) — external — Open-source agent that operates a real browser
...
- `.gitignore`, `LICENSE`, `README.md` — repo root files
- `advanced_ai_agents/` — production-style single- and multi-agent apps (tools, memory, multi-step reasoning), incl. `multi_agent_apps/agent_teams/`, `autonomous_game_playing_agent_apps/`
- `advanced_llm_apps/` — memory-enabled LLM apps, "Chat with X" tutorials, LLM optimization tools, fine-tuning tutorials
- `agent_skills/` — installable Claude/Codex/Cursor agent skills (each with CI-gated security + eval checks)
- `ai_agent_framework_crash_course/` — deep-dive crash courses for Google ADK and OpenAI Agents SDK
- `always_on_agents/` — scheduled/event-driven background agents
- `docs/` — banner and gallery image assets
- `generative_ui_agents/` — agents that render interactive UI (kanban, dashboards, shadcn components)
- `mcp_ai_agents/` — agents wired to Model Context Protocol servers
- `rag_tutorials/` — RAG pipeline implementations, simple to agentic/multi-source
- `starter_ai_agents/` — single-file agents runnable with just an API key
- `voice_ai_agents/` — real-time speech-in/speech-out agents
